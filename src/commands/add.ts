...
import { ok, xtry } from '@zokugun/xtry/async';
...
import { validateNewPackage } from '../flow/validators/validate-new-package.js';
...
export async function add(specs: string[], inputOptions: CLIOptions = {}): Promise<void> {
	...
	const result = await resolveAndRunFlow(toIterator(ok, requests), true, true, OperationType.Install, validateNewPackage, targetPath, commonFlow, updateInstallConfig, config, global, options);
	if(result.fails) {
