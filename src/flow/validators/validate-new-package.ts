...
export function validateNewPackage(requestPackage: PackageManifest, installConfig: InstallConfig, _global: Global, options: Options): DResult<{ operationMode?: OperationMode } | undefined> {
	if(options.force) {
		return OK_VALID;
	}
	...
	if(options.skip) {
		if(options.verbose) {
			logger.debug('The new artifact is already present, skipping...');
		}

		return OK_UNDEFINED;
	}
	else {
		return err('The new artifact is already present.');
	}
}
